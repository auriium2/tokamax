...
## Experimental SSD assignment in this fork

This fork includes a verified float32 XLA reference for Mamba2 SSD forward and
an unimplemented Pallas backend. Implementing that backend is the assignment.

`ssd(x, log_decay, b, c, initial_state, implementation="xla")` returns
...
The reference supports nonzero initial state and dimension tails. The assignment
excludes decoding APIs, gradients, and the surrounding Mamba2 layer.

`implementation="pallas"` selects the agent-owned backend in
`tokamax/_src/ops/ssd/pallas/`. At this baseline it raises `NotImplementedError`,
with no XLA fallback. That package may use Pallas-Triton or Pallas-Mosaic and
contain multiple kernels, helper modules, layouts, and tuning configurations.
No particular algorithm, launch grid, or tuning search space is prescribed.
Use `uv sync --locked` from this fork checkout for local XLA development, or
`uv sync --locked --extra cuda` on the target Linux GPU host.
